...
def load_events(filename):
    ...
    if tree_b == None: raise Exception('Can not find tree: {}'.format('TreeB'))

    # Get values
    events_s = np.zeros((tree_s.GetEntries(), 4))
    labels_s = np.zeros((tree_s.GetEntries(), 2))

    events_b = np.zeros((tree_b.GetEntries(), 4))
    labels_b = np.zeros((tree_b.GetEntries(), 2))
...
def main(args):
    ...
    # Train model
    model = build_model()
    model.fit(events, labels, batch_size=100, epochs=10, validation_split=0.2)
